...
def about(request):
    abouttab = CompanyProfile.objects.get(pk=3)
...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        if contact.is_valid():
            contact.save()
            messages.success(request,'Your message has been sent successfully, one of our representative would get back to you shortly!!')
    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                basket.save()
                messages.success(request, format_html('one item added..... <a href="http://54.167.189.240/cart">View Cart</a>'))
                return redirect('home')
            else:
                newitem = Cart()
                ...
                newitem.paid = False
                newitem.save()
                messages.success(request, format_html('one item added..... <a href="http://54.167.189.240/cart" style="color:red;">View Cart</a>'))
                return redirect('home')
        else:
            newcart = Cart()
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_a1c8d758a1269536b17d719f18d0fc92f50d8a4b' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://54.167.189.240/callback' #payment confirmation page
        ...
        total = float(request.POST['total']) * 100 #total amountto be charged from the client card
        ...
        data = {'refrence':ref, 'amount':int(total), 'email':user.email, 'callback_url': cburl, 'order_number':order_no, 'currency': 'NGN'}
